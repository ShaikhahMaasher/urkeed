...
class nxLiveSearch {
	...
	function get_templates($template, $type='')
	{
		...
		switch($type) {
			case 'more':
				$template_post = '<a href="{search_url_escaped}"><span class="sf_text">'. __('See more results for', 'ispirit'). '"{search_value}"</span><span class="sf_small">' . __('Displaying top {total} results', 'ispirit').'</span></a>';
				break;
			case 'taxonomy':
			...
			case 'post_type':
				...
				$template_post = '<a href="{post_link}">{post_image_html}<span class="sf_text">{post_title}</span><span class="prod-price">{wp_currency} {price}</span></a>';
				break;
			default:
